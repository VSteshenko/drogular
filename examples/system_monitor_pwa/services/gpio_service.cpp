...
GpioService::GpioService(
    std::shared_ptr<GpioProvider> provider,
      ...
      refreshInterval_(refreshInterval)
{
    if (refreshInterval_ < std::chrono::milliseconds::zero()) {
        throw std::invalid_argument("GpioService refresh interval cannot be negative");
    }
...
GpioSnapshot GpioService::snapshot() {
    ...
    try {
        GpioSnapshot snapshot;
        const auto chips = provider_->chips();
        snapshot.chips.reserve(chips.size());

        for (const auto& chip : chips) {
            GpioChipSnapshot chipSnapshot;
            chipSnapshot.chip = chip;
            chipSnapshot.lines = provider_->lines(chip.name);
            snapshot.chips.push_back(std::move(chipSnapshot));
        }
...
GpioServiceStatistics GpioService::statistics() const {
    std::lock_guard lock(mutex_);
...
bool GpioService::available() const noexcept {
